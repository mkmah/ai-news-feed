...
class YoutubeScraper:
    ...
    async def get_latest_videos(self, channel_id: str, hours: int = 24) -> List[YoutubeVideo]:
        ...
        for entry in feed.entries:
            ...
            if video_date > cut_off_time:
                videos.append(
                    YoutubeVideo(
                        id=self._extract_video_id(entry.link),
                        title=entry.title,
    ...
    async def scrape_channel(self, channel_id: str, hours: int = 24) -> List[YoutubeVideo]:
        videos = await self._get_latest_videos(channel_id, hours)

        tasks = [self._get_transcript(video.id) for video in videos]
